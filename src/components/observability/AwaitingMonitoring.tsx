...
interface AwaitingMonitoringProps {
  title?: string;
  detail?: string;
  icon?: LucideIcon;
  action?: ReactNode;
...
/**
 * Canonical honest placeholder for the Observability Center. No metric, log
 * line, latency figure or deployment record is ever fabricated.
 */
export function AwaitingMonitoring({
  title = "Awaiting Live Platform Monitoring",
  detail = "This surface populates from the monitoring backend (Prometheus / OpenTelemetry). Nothing shown here is simulated.",
  icon: Icon = Activity,
  action,
  compact = false,
  className,
}: AwaitingMonitoringProps) {
  if (compact) {
    return (
      <div
        role="status"
